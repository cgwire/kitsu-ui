...
import { sortByName } from '../../libs/sorting'
import { capitalize } from '../../libs/string'
import EditSearchFilterModal from '../modals/EditSearchFilterModal.vue'
import EditSearchFilterGroupModal from '../modals/EditSearchFilterGroupModal.vue'
  ...
  methods: {
    ...
    async confirmEditFilterGroup(filterGroup) {
      try {
        ...
        if (!filterGroup.id) {
          await this[
            `save${capitalize(this.type)}SearchFilterGroup`
          ](filterGroup)
        } else {
          await this.actions.updateSearchFilterGroup(filterGroup)
        }
    ...
    async confirmEditSearch(searchFilter) {
      try {
        ...
        await this.actions.updateSearchFilter(searchFilter)
    ...
    async removeGroup(filterGroup) {
      try {
        await this[
          `remove${capitalize(this.type)}SearchFilterGroup`
        ](filterGroup)
      } catch (err) {
        console.error(err)
      }
